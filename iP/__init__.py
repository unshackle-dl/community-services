...
class iP(Service):
        ...
        if self.range and self.range.name == "HLG":
            if not self.config.get("certificate"):
                raise CertificateMissingError(
                    "HLG/H.265 tracks cannot be requested without a TLS certificate."
                )
    ...
    def search(self) -> Generator[SearchResult, None, None]:
        r = self.session.get(
            self.config["endpoints"]["search"], params={"q": self.title}
        )
        r.raise_for_status()
    ...
    def get_titles(self) -> Movies | Series:
        ...
        if not match:
            raise ValueError(
                "Could not parse ID from title - is the URL/ID format correct?"
            )
        ...
        if data is None:
            raise MetadataError(
                f"Metadata not found for '{pid}'. If it's an episode, use the full URL."
            )
            ...
            return Movies(
                [
                    Movie(
                        ...
                        year=(movie_data.get("release_date_time", "") or "").split("-")[
                            0
                        ],
        ...
        # It's a full series
        seasons = [
            self.get_data(pid, x["id"]) for x in data.get("slices") or [{"id": None}]
        ]
        episode_ids = [
    ...
    def get_tracks(self, title: Movie | Episode) -> Tracks:
        ...
        if not versions:
            raise NoStreamsAvailableError(
                "No available versions for this title were found."
            )
        ...
        if not connections:
            ...
            raise NoStreamsAvailableError(
                "Selection unavailable. Title may be missing or geo-blocked."
            )
        ...
        return self._select_tracks(media, title.language)
    ...
    def _get_available_versions(self, pid: str) -> list[dict]:
        ...
        r.raise_for_status()
        match = re.search(
            r"window\.__IPLAYER_REDUX_STATE__\s*=\s*(.*?);\s*</script>", r.text
        )
        if match:
            redux_data = json.loads(match.group(1))
            redux_versions = redux_data.get("versions")
            versions = (
                redux_versions.values()
                if isinstance(redux_versions, dict)
                else redux_versions
            )
    ...
    def _select_best_media(self, connections: list[list[dict]]) -> list[dict]:
        ...
        self.log.debug(
            f"Available resolutions (p): {heights}. Selecting highest: {highest_height}p."
        )
    ...
    def _select_tracks(self, media: list[dict], lang: str):
        for video_stream_info in (m for m in media if m.get("kind") == "video"):
            connections = sorted(
                video_stream_info["connection"], key=lambda x: x.get("priority", 99)
            )
            ...
            if self.vcodec == "H.265":
                ...
                connection = next(
                    (
                        ...
                        if c["supplier"] == "mf_akamai"
                        and c["transferFormat"] == "dash"
                    ),
                    None,
                )
        ...
        if connection["transferFormat"] == "dash":
            ...
            raise ValueError(
                f"Unsupported transfer format: {connection['transferFormat']}"
            )
        ...
        for caption in [x for x in media if x["kind"] == "captions"]:
            connection = min(caption["connection"], key=lambda x: x["priority"])
            tracks.add(
                Subtitle(
    ...
    def check_all_versions(self, vpid: str) -> list:
        ...
        if self.vcodec == "H.265":
            ...
            with tempfile.NamedTemporaryFile(
                mode="w+b", delete=False, suffix=".pem"
            ) as cert_file:
