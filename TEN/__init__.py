...
AD_HOST = "redirector.googlevideo.com"
...
class TEN(Service):
    ...
    Notes:
        ...
        - Google DAI stitches ads into the stream. The track's OnSegmentFilter drops those
          segments, which keeps the cuepoint chapter timestamps in step with the output.
        - The master playlist does not advertise every quality the CDN serves. To reach the higher
          ones, the service rewrites the bitrate in the media playlist URIs and hands the result to
          the downloader with `from_file`.
    """
    ...
    def get_tracks(self, title: Movie | Episode) -> Tracks:
        ...
        payload = {"auth-token": dai_auth} if dai_auth else {}
        ...
        tracks = HLS.from_url(manifest_url, self.session).to_tracks(
    ...
    def get_chapters(self, title: Movie | Episode) -> Chapters:
        ...
        for cue in cue_points:
            chapters.extend(
                (
                    Chapter(timestamp=float(cue["start_float"])),
                    Chapter(timestamp=float(cue["end_float"])),
                )
            )
    ...
    @staticmethod
    def _is_ad_segment(segment: Any) -> bool:
        return AD_HOST in (segment.uri or "")
    ...
    def _check_and_add_track(
        ...
        # Ads stay in the playlist for OnSegmentFilter to drop. Only content segments carry the
        # source bitrate in their path, so this replace leaves the ads alone.
        modified_playlist_text = playlist_text.replace(
            f"-{source_bitrate}", f"-{quality_info['bitrate']}"
        )
        playlist_obj = m3u8.loads(modified_playlist_text, uri=playlist_uri)

        content_segments = [
            s for s in playlist_obj.segments if not self._is_ad_segment(s)
        ]
        if not content_segments:
            return None

        # The downloader loads a from_file playlist off disk, where a relative URI would resolve
        # against the cache directory. Absolutise everything it dereferences.
        for segment in playlist_obj.segments:
            segment.uri = urljoin(playlist_uri, segment.uri)
            if segment.init_section:
                segment.init_section.uri = urljoin(
                    playlist_uri, segment.init_section.uri
                )
        for key in filter(None, playlist_obj.keys):
            if key.uri:
                key.uri = urljoin(playlist_uri, key.uri)

        if self._head_request(content_segments[0].uri) == 200:
            playlist_file = (
                config.directories.cache
                / "TEN"
                / f"playlist_{quality_info['quality']}.m3u8"
            )
    ...
    def _add_tracks(self, tracks: Tracks) -> Tracks:
        ...
        with ThreadPoolExecutor(max_workers=len(qualities_to_check)) as executor:
            future_to_track = {
                executor.submit(
                    self._check_and_add_track, best_track, quality, source_bitrate
                ): quality
            ...
            for episode in result:
                ...
                titles.append(
                    Episode(
                        ...
                        season=int(sea_number)
                        if sea_number and sea_number.isdigit()
                        else 0,
                        number=int(ep_number)
                        if ep_number and ep_number.isdigit()
                        else 0,
    ...
    def _signature_header(self, url: str) -> str:
        ...
        message = f"{timestamp}:{url}".encode()
